...
class FitModel(object):
    ...
    """
    def __init__(self, bg_order=1, n_peaks=1):
        self.model_params = np.array([])
        self.model_bank = {}
        self.build(bg_order, n_peaks)

    def build(self, bg_order, n_peaks):
        """!
        ...
        for i in range(n_peaks):
            name = "gauss_" + str(i)
            self.add_model(peak.GaussModel(name=name))
    ...
    def add_model(self, in_model):
        ...
        self.model_bank[in_model.name]["idxs"] = range(current_nparams, current_nparams + input_model_nparams)
    ...
    def eval(self, x):
        ...
        output = np.zeros(len(x))
        for model_name, model in iteritems(self.model_bank):
            output += model["model"].opti_eval(x, self.model_params[model["idxs"]])
        return output
